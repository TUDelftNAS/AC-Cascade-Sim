...
%you need load update_case_data-fuzzy.m > this function is changed comparing
...
clc
close all
addpath('D:.....\matpower') % add path to your matpower
...
mpopt = mpoption( 'out.all', 0, 'pf.nr.max_it', 50); %to turn of the writings on the matlab command window and to inrease the iteration number to 50 ( default is 10)
...
while results.success ~=1
    if sum(mpc.bus(:,3))>0.1 %if there is load to shed (assuming the min. load is 0.1 to prevent infinite loop)
...
lineThreat= (1:num_links)'; %Needs to be input, column vector
lineThreat=lineThreat(S_branch>thres);
...
tic
for e=1:length(lineThreat)
  
    mpc=base_case; %load the base case unchanged
    results=results_base;
   
    %Check the warning message regarding the attacks
...
line_flow(e,1)=S_branch(lineThreat(e,1),2);
%find the effect of initial atack
[islands_current ,island_cont, tripped_due_to_fuzzy]= Update_case_data_fuzzy(mpc, Branch_loading);
...
%in the first column, the continuity information of teh islands in the
%second column
...
for i=1:size(island_cont,2) %for all islands that is formed
    if island_cont(1,i)>0 %is island continues
...
for i=1:size(islands_inter,2) %for all islands that continue
    ...
    if any(Branch_loading{1,i}>=100)%meaning there is an overloaded link
        ...
        %find new islands
        [islands_current_dummy ,island_cont_dummy, tripped_due_to_fuzzy ]= Update_case_data_fuzzy(results_inter(1,i), Branch_loading{1,i});   
        islands_current= [islands_current islands_current_dummy ];
        island_cont=[island_cont island_cont_dummy];
        links_removed_dum=[results_inter(1,i).branch((Branch_loading{1,i}>=100),RATE_B); tripped_due_to_fuzzy];
