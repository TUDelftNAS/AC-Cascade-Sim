...
%you need load update_case_data_inertia.m > this function is changed comparing
...
mpopt = mpoption( 'out.all', 0, 'pf.nr.max_it', 10); %to turn of the writings on the matlab command window and to inrease the iteration number to 50 ( default is 10)
...
while results.success ~=1
    ...
    mpc= load_shedding_inertia(mpc,0.01); 
...
[islands_current ,island_cont]= Update_case_data_inertia(mpc, Branch_loading);
...
for i=1:size(islands_inter,2) %for all islands, we will do load flow now
    ...
    islands_inter(1,i)= load_shedding_inertia(islands_inter(1,i),0.1);
        end
        if sum(islands_inter(1,i).bus(:,3))<=1%if there is no load to shed
...
for i=1:size(islands_inter,2) %for all islands that continue
    ...
    if any(Branch_loading{1,i}>=100)%meaning there is an overloaded link
        ...
        %find new islands
        [islands_current_dummy ,island_cont_dummy ]= Update_case_data_inertia(results_inter(1,i), Branch_loading{1,i});   
